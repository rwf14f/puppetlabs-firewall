...
describe 'puppet resource firewall command' do
  ...
  context 'when accepts rules utilizing the statistic module' do
    before :all do
      ...
      run_shell('iptables -t nat -A POSTROUTING -d 1.2.3.4/32 -o eth0 -m statistic --mode nth --every 2 -j SNAT --to-source 2.3.4.5', acceptable_exit_codes: [0, 1, 2])
  ...
  context 'when accepts rules with -m ttl' do
    before :all do
      iptables_flush_all_tables
      run_shell('iptables -t nat -A OUTPUT -s 10.0.0.0/8 -p tcp -m ttl ! --ttl-eq 42 -j REDIRECT --to-ports 12299')
    end
